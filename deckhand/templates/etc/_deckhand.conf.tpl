...
{{ if not .keystone_authtoken.auth_uri }}#{{ end }}auth_uri = {{ .keystone_authtoken.auth_uri | default "<None>" }}

# API version of the admin Identity API endpoint. (string value)
{{ if not .keystone_authtoken.auth_version }}#{{ end }}auth_version = {{ .keystone_authtoken.auth_version | default "<None>" }}

# Do not handle authorization requests within the middleware, but delegate the
# authorization decision to downstream WSGI components. (boolean value)
{{ if not .keystone_authtoken.delay_auth_decision }}#{{ end }}delay_auth_decision = {{ .keystone_authtoken.delay_auth_decision | default "false" }}

# Request timeout value for communicating with Identity API server. (integer
# value)
{{ if not .keystone_authtoken.http_connect_timeout }}#{{ end }}http_connect_timeout = {{ .keystone_authtoken.http_connect_timeout | default "<None>" }}

# How many times are we trying to reconnect when communicating with Identity API
# Server. (integer value)
{{ if not .keystone_authtoken.http_request_max_retries }}#{{ end }}http_request_max_retries = {{ .keystone_authtoken.http_request_max_retries | default "3" }}

# Request environment key where the Swift cache object is stored. When
# auth_token middleware is deployed with a Swift cache, use this option to have
# the middleware share a caching backend with swift. Otherwise, use the
# ``memcached_servers`` option instead. (string value)
{{ if not .keystone_authtoken.cache }}#{{ end }}cache = {{ .keystone_authtoken.cache | default "<None>" }}

# Required if identity server requires client certificate (string value)
{{ if not .keystone_authtoken.certfile }}#{{ end }}certfile = {{ .keystone_authtoken.certfile | default "<None>" }}

# Required if identity server requires client certificate (string value)
{{ if not .keystone_authtoken.keyfile }}#{{ end }}keyfile = {{ .keystone_authtoken.keyfile | default "<None>" }}

# A PEM encoded Certificate Authority to use when verifying HTTPs connections.
# Defaults to system CAs. (string value)
{{ if not .keystone_authtoken.cafile }}#{{ end }}cafile = {{ .keystone_authtoken.cafile | default "<None>" }}

# Verify HTTPS connections. (boolean value)
{{ if not .keystone_authtoken.insecure }}#{{ end }}insecure = {{ .keystone_authtoken.insecure | default "false" }}

# The region in which the identity server can be found. (string value)
{{ if not .keystone_authtoken.region_name }}#{{ end }}region_name = {{ .keystone_authtoken.region_name | default "<None>" }}
...
{{ if not .keystone_authtoken.signing_dir }}#{{ end }}signing_dir = {{ .keystone_authtoken.signing_dir | default "<None>" }}

# Optionally specify a list of memcached server(s) to use for caching. If left
# undefined, tokens will instead be cached in-process. (list value)
# Deprecated group/name - [keystone_authtoken]/memcache_servers
{{ if not .keystone_authtoken.memcached_servers }}#{{ end }}memcached_servers = {{ .keystone_authtoken.memcached_servers | default "<None>" }}

# In order to prevent excessive effort spent validating tokens, the middleware
# caches previously-seen tokens for a configurable duration (in seconds). Set to
# -1 to disable caching completely. (integer value)
{{ if not .keystone_authtoken.token_cache_time }}#{{ end }}token_cache_time = {{ .keystone_authtoken.token_cache_time | default "300" }}
...
# Its value may be silently ignored in the future.
# Reason: PKI token format is no longer supported.
{{ if not .keystone_authtoken.revocation_cache_time }}#{{ end }}revocation_cache_time = {{ .keystone_authtoken.revocation_cache_time | default "10" }}
...
{{ if not .keystone_authtoken.memcache_security_strategy }}#{{ end }}memcache_security_strategy = {{ .keystone_authtoken.memcache_security_strategy | default "None" }}

# (Optional, mandatory if memcache_security_strategy is defined) This string is
# used for key derivation. (string value)
{{ if not .keystone_authtoken.memcache_secret_key }}#{{ end }}memcache_secret_key = {{ .keystone_authtoken.memcache_secret_key | default "<None>" }}

# (Optional) Number of seconds memcached server is considered dead before it is
# tried again. (integer value)
{{ if not .keystone_authtoken.memcache_pool_dead_retry }}#{{ end }}memcache_pool_dead_retry = {{ .keystone_authtoken.memcache_pool_dead_retry | default "300" }}

# (Optional) Maximum total number of open connections to every memcached server.
# (integer value)
{{ if not .keystone_authtoken.memcache_pool_maxsize }}#{{ end }}memcache_pool_maxsize = {{ .keystone_authtoken.memcache_pool_maxsize | default "10" }}

# (Optional) Socket timeout in seconds for communicating with a memcached
# server. (integer value)
{{ if not .keystone_authtoken.memcache_pool_socket_timeout }}#{{ end }}memcache_pool_socket_timeout = {{ .keystone_authtoken.memcache_pool_socket_timeout | default "3" }}

# (Optional) Number of seconds a connection to memcached is held unused in the
# pool before it is closed. (integer value)
{{ if not .keystone_authtoken.memcache_pool_unused_timeout }}#{{ end }}memcache_pool_unused_timeout = {{ .keystone_authtoken.memcache_pool_unused_timeout | default "60" }}

# (Optional) Number of seconds that an operation will wait to get a memcached
# client connection from the pool. (integer value)
{{ if not .keystone_authtoken.memcache_pool_conn_get_timeout }}#{{ end }}memcache_pool_conn_get_timeout = {{ .keystone_authtoken.memcache_pool_conn_get_timeout | default "10" }}

# (Optional) Use the advanced (eventlet safe) memcached client pool. The
# advanced pool will only work under python 2.x. (boolean value)
{{ if not .keystone_authtoken.memcache_use_advanced_pool }}#{{ end }}memcache_use_advanced_pool = {{ .keystone_authtoken.memcache_use_advanced_pool | default "false" }}

# (Optional) Indicate whether to set the X-Service-Catalog header. If False,
# middleware will not ask for service catalog on token validation and will not
# set the X-Service-Catalog header. (boolean value)
{{ if not .keystone_authtoken.include_service_catalog }}#{{ end }}include_service_catalog = {{ .keystone_authtoken.include_service_catalog | default "true" }}
...
# value)
{{ if not .keystone_authtoken.enforce_token_bind }}#{{ end }}enforce_token_bind = {{ .keystone_authtoken.enforce_token_bind | default "permissive" }}
...
# Its value may be silently ignored in the future.
# Reason: PKI token format is no longer supported.
{{ if not .keystone_authtoken.check_revocations_for_cached }}#{{ end }}check_revocations_for_cached = {{ .keystone_authtoken.check_revocations_for_cached | default "false" }}
...
# Its value may be silently ignored in the future.
# Reason: PKI token format is no longer supported.
{{ if not .keystone_authtoken.hash_algorithms }}#{{ end }}hash_algorithms = {{ .keystone_authtoken.hash_algorithms | default "md5" }}
...
# check. (list value)
{{ if not .keystone_authtoken.service_token_roles }}#{{ end }}service_token_roles = {{ .keystone_authtoken.service_token_roles | default "service" }}
...
# (boolean value)
{{ if not .keystone_authtoken.service_token_roles_required }}#{{ end }}service_token_roles_required = {{ .keystone_authtoken.service_token_roles_required | default "false" }}

# Authentication type to load (string value)
# Deprecated group/name - [keystone_authtoken]/auth_plugin
{{ if not .keystone_authtoken.auth_type }}#{{ end }}auth_type = {{ .keystone_authtoken.auth_type | default "<None>" }}

# Config Section from which to load plugin specific options (string value)
{{ if not .keystone_authtoken.auth_section }}#{{ end }}auth_section = {{ .keystone_authtoken.auth_section | default "<None>" }}
